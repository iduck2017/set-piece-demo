...
describe('child', () => {
    ...
    test('reset', () => {
        aaronson.origin.child.subordinates[0] = syme;
        expect(aaronson.child.subordinates[0]).toBe(syme);
    })

    
    test('pop', () => {
        const result = obrien.origin.child.subordinates.pop();
    ...
    test('set', () => {
        aaronson.origin.child.subordinates[0] = julia;
        expect(aaronson.child.subordinates.length).toBe(1);
        expect(aaronson.child.subordinates[0]).toBe(julia);
    })

    test('copy', () => {
        aaronson.origin.child.subordinates.push(syme, syme);
    ...
    test('shift', () => {
        const result = obrien.origin.child.subordinates.shift();
        expect(result).toBe(winston);
        expect(obrien.child.subordinates.length).toBe(0);
    })

    test('unshift', () => {
        aaronson.origin.child.subordinates.unshift(winston);
    ...
    test('fill', () => {
        aaronson.origin.child.subordinates.fill(syme);
        expect(aaronson.child.subordinates.length).toBe(4);
        expect(aaronson.child.subordinates[0]).toBe(syme);
        expect(aaronson.child.subordinates[1]).toBe(syme);
        expect(aaronson.child.subordinates[2]).toBe(syme);
    ...
    test('splice', () => {
        aaronson.origin.child.subordinates.splice(2, 2, winston, julia);
